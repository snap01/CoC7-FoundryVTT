...
Hooks.on('renderTileHUD', (app, html, data) => {
    ...
    $('<div>')
        ...
        .append($('<img>').attr({
            src: 'systems/CoC7/lib/monks-active-tiles/img/power-button.svg',
            width: '36',
            height: '36',
...
Hooks.on("renderWallConfig", async (app, html, options) => {
    if (setting("allow-door")) {
        ...
        let triggerData = mergeObject({ tilename: tilename, showtagger: game.modules.get('tagger')?.active, poweredBy: !game.modules.get('monks-active-tiles')?.active }, (app.object.data.flags['monks-active-tiles'] || {}));
        let wallHtml = await renderTemplate("systems/CoC7/lib/monks-active-tiles/templates/wall-config.html", triggerData);
